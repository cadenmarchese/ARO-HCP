...
var _ = Describe("Cluster Etcd Managed HSM Encryption", func() {
	timeBombDeadline := framework.V20261003PreviewDeploymentDeadline

	It("should create a cluster with Managed HSM keyEncryptionKeyUrl and verify etcd encryption works",
		...
		func(ctx context.Context) {
			...
			apiAvailable, err := tc.IsHCPAPIVersionAvailable(ctx, "2026-10-03-preview")
			Expect(err).NotTo(HaveOccurred(), "failed to check v20261003preview API version availability")
			if !apiAvailable {
				if time.Now().After(timeBombDeadline) {
					Fail(fmt.Sprintf("API version 2026-10-03-preview should be fully available by %s", timeBombDeadline.Format(time.RFC3339)))
				}
				Skip("API version 2026-10-03-preview is not fully available in this environment")
			}
			...
			clusterParams := framework.NewDefaultClusterParams20261003()
			...
			By("creating HCP cluster with Managed HSM keyEncryptionKeyUrl via v20261003preview")
			err = tc.CreateHCPClusterFromParam20261003(ctx,
				GinkgoLogr,
				*resourceGroup.Name,
				clusterParams,
				nil,
			...
			if isAPINotDeployedError(err) {
				if time.Now().Before(timeBombDeadline) {
					Skip(fmt.Sprintf("v20261003preview API not yet deployed; skipping until %s", timeBombDeadline.Format(time.RFC3339)))
				}
				Fail(fmt.Sprintf("v20261003preview API still not deployed as of %s deadline", timeBombDeadline.Format(time.RFC3339)))
			}
			Expect(err).NotTo(HaveOccurred(), "failed to create HCP cluster %q with mHSM keyEncryptionKeyUrl", customerClusterName)

			By("verifying keyEncryptionKeyUrl was stored correctly via GET")
			cluster, err := framework.GetHCPCluster20261003(ctx,
				tc.Get20261003ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient(),
				*resourceGroup.Name,
				customerClusterName,
			)
			...
			Expect(cluster.Properties.Etcd.DataEncryption.CustomerManaged.Kms.KeyEncryptionKeyURL).NotTo(BeNil(), "cluster %q Kms.KeyEncryptionKeyURL was nil", customerClusterName)
			Expect(*cluster.Properties.Etcd.DataEncryption.CustomerManaged.Kms.KeyEncryptionKeyURL).To(Equal(keyID),
				"cluster %q keyEncryptionKeyUrl should match the mHSM key ID", customerClusterName)
			Expect(strings.Contains(*cluster.Properties.Etcd.DataEncryption.CustomerManaged.Kms.KeyEncryptionKeyURL, ".managedhsm.azure.net")).To(BeTrue(),
				"cluster %q keyEncryptionKeyUrl should contain .managedhsm.azure.net", customerClusterName)

			By("getting admin credentials for the cluster")
			adminRESTConfig, err := tc.GetAdminRESTConfigForHCPCluster20261003(
				ctx,
				tc.Get20261003ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient(),
				*resourceGroup.Name,
				customerClusterName,
